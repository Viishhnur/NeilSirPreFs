...
public class Day26P2{
    private static boolean canDistribute(int[] gold,int m,int maxWt){
        int cnt = 0;
        for(int goldBarWt : gold){
            cnt += (goldBarWt / maxWt);
    ...
    private static int maxGoldThatCanBeDistributed(int[] gold,int n,int m){
        int low = 1;
        int high = Arrays.stream(gold).max().orElse(gold[0]);
        int res = 0;
        while(low <= high){
            int mid = low + (high - low)/2;

            if(canDistribute(gold,m,mid)){
                res = mid;
                // try for a larger minimum
                low = mid + 1;
            }
            else{
                high = mid - 1;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxGoldThatCanBeDistributed(gold,n,m));
